...
setwd("/Users/uwvirongs/Documents/Michelle/tprk_files/new_tprk/demuxed/20200820_pacbio/repeat_dotline_plots/")
setwd('/Users/uwvirongs/Documents/Michelle/tprk_files/tprk_redo_v6/comparing_highs/AS18_redo/')
path<-getwd()
#####
...
if (ref_sample == FALSE) {
  ...
  foreach(j=1:(length(sample_names))) %dopar% {
    if (sample_names[j] != ref_sample) {
      ...
      rfcol <- "Ill_AS18_RelativeFreq"
      rfcol2 <- "Ill_AS18_high_RelativeFreq"
      ...
      sortedIllumina$Sample[sortedIllumina$Sample == rfcol] <- "AS18"
      sortedIllumina$Sample[sortedIllumina$Sample == rfcol2] <- "AS18_high"
      ...
      h1 <- h + colScale
      ggsave("AS10_Repeat_Comparison_Plots.pdf",plot=h1,width=5,height=4,units="in")
