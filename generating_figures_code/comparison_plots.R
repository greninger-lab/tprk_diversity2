...
setwd('/Users/uwvirongs/Documents/Michelle/tprk_files/tprk_redo_v6/')
path<-getwd()
...
alldatafilt <- read.csv("allreads_ee1.csv",header=TRUE,sep=",",stringsAsFactors = FALSE)

alldatafilt <- read.csv("AS11_Illumina_squared.csv",header=TRUE,sep=",",stringsAsFactors = FALSE)
...
#illrf_col <- "PB_AS12_RelativeFreq"
#pbrf_col <- "PB_AS11.repeat_RelativeFreq"

#illrf_col <- (paste("PB_",sample_name,"_RelativeFreq",sep=""))
#pbrf_col <- (paste("PB_",sample_name,"_repeat_RelativeFreq",sep=""))
#illcount_col <- (paste("PB_",sample_name,"_Count",sep=""))
#pbcount_col <- (paste("PB_",sample_name,"_repeat_Count",sep=""))
pbcount_col <- (paste("PB_",sample_name,"_Count",sep=""))
illcount_col <- (paste("Ill_",sample_name,"_Count",sep=""))

pbcount_col <- (paste("Ill2_",sample_name,"_Count",sep=""))
pbrf_col <- (paste("Ill2_",sample_name,"_RelativeFreq",sep=""))
...
# Remove 2 column NAs
vs_relative_freq_filt <- vs_relative_freq_filt[!with(vs_relative_freq_filt,is.na(Ill_AS11_RelativeFreq)& is.na(Ill2_AS11_RelativeFreq)),]
vs_relative_freq_filt[is.na(vs_relative_freq_filt)] <- 0
...
# Plots for filtered.
plot_filt <- ggplot(vs_relative_freq_filt,aes(x=vs_relative_freq_filt[[illrf_col]],y=vs_relative_freq_filt[[pbrf_col]],color=Region)) +
  ...
  xlab(label="Illumina1 Frequency") + ylab(label="Illumina2 Frequency")  + 
...
ggsave(paste(sample_name,"Illumina_vs_Illumina.pdf",sep="_"),path="./",plot=plot_filt,width=6,height=3,units="in")
...
mylegend<-g_legend(plot_filt)
figure <- grid.arrange(arrangeGrob(plot + theme(legend.position="right"),
                                   plot_filt + theme(legend.position="right"),
                                   nrow=1),
