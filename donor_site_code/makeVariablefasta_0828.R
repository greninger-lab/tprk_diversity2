...
setwd("/Users/uwvirongs/Documents/Michelle/tprk_files/tprk_redo_five_billion_maybe_final/donor_sites")
setwd("/Users/uwvirongs/Documents/Michelle/tprk_files/tprk_redo_v6/donor_trimmed/")
...
for (i in c(1:length(filenames))){
  ...
  sample<-as.character(strsplit(strsplit(filenames[i],"_")[[1]][1],"\\.")[[1]][1])
...
## Chinese
###
syscommand <- "cat SRR*_variable.fasta > chinese_all-variable.fasta"
system(syscommand)

allvariable <- read.fasta("chinese_all-variable.fasta")
...
writeXStringSet(dnaString, "chinese_all-variable_edited.fasta",append=FALSE,format="fasta")

syscommand_amin <- "blastn -db tprDlocus.fasta -dust no -soft_masking false -query chinese_all-variable_edited.fasta -evalue 1e1 -word_size 10 -ungapped -perc_identity 100 -penalty -10 -outfmt 6 -max_hsps 3 -subject_besthit -out chinese_Step3_search.blast"
system(syscommand_amin)

blasthits <- read.table("chinese_Step3_search.blast",
...
chinese_blasthits_100 <- blasthits[blasthits$count > 50 & blasthits$percentage > 0.2,]
chinese_blasthits_100 <- chinese_blasthits_100[order(-chinese_blasthits_100$length),]


#####
## ours
####
syscommand <- "cat *_validated_variable.fasta > our_all-variable.fasta"
system(syscommand)
allvariable <- read.fasta("our_all-variable.fasta")
allvariable$region <- str_split_fixed(allvariable$seq.name,"_",4)[,2]
allvariable[allvariable$region=="V3",]$seq.text <- str_sub(allvariable[allvariable$region=="V3",]$seq.text, end=-24)
allvariable[allvariable$region=="V2",]$seq.text <- str_sub(allvariable[allvariable$region=="V2",]$seq.text, end=-15)
allvariable[allvariable$region=="V5",]$seq.text <- str_sub(allvariable[allvariable$region=="V5",]$seq.text, end=-14)
...
dnaString <- BStringSet(allvariable$seq.text)
names(dnaString) = paste0(allvariable$seq.name)
writeXStringSet(dnaString, "our_all-variable_edited.fasta",append=FALSE,format="fasta")

### Blast the edited allvariable fasta against our 17.2kb locus
syscommand_amin <- "blastn -db tprDlocus.fasta -dust no -soft_masking false -query our_all-variable_edited.fasta -evalue 1e1 -word_size 10 -ungapped -perc_identity 100 -penalty -10 -outfmt 6 -max_hsps 3 -subject_besthit -out our_Step3_search.blast"
system(syscommand_amin)

### Set up the blasthits file
our_blasthits <- read.table("our_Step3_search.blast",
                        col.names = c("qseqid","sseqid","pident","length","mismatch",
                                      "gapopen","qstart","qend","sstart","send","evalue","bitscore"))

count <- as.numeric(str_split_fixed(our_blasthits$qseqid,"_",4)[,3])
sample <- str_split_fixed(our_blasthits$qseqid,"_",4)[,1]
region <- str_split_fixed(our_blasthits$qseqid,"_",4)[,2]
interval<-paste(our_blasthits$sstart,our_blasthits$send,sep="-")
our_blasthits <- dplyr::mutate(our_blasthits,count=count,sample=sample,interval=interval,region=region)
our_blasthits <- our_blasthits %>% dplyr::group_by(sample, region)  %>% dplyr::mutate(regionsamplecounts = sum(count))
our_blasthits <- ungroup(our_blasthits)
our_blasthits <- dplyr::mutate(our_blasthits, percentage = round(count / regionsamplecounts * 100,3))

our_blasthits_100 <- our_blasthits[our_blasthits$count > 50 & our_blasthits$percentage > 0.1,]
our_blasthits_100 <- our_blasthits_100[order(-our_blasthits_100$length),]
...
## Filter our blasthits above 0.1, Chinese on 0.2
...
chinese_blasthits_100
our_blasthits_100

blasthits_100 <- rbind(chinese_blasthits_100,our_blasthits_100)

system("cat chinese_all-variable_edited.fasta our_all-variable_edited.fasta > all-variable_edited.fasta")
system("cat chinese_all-variable.fasta our_all-variable.fasta > all-variable.fasta")
...
variableRegions_pre_new_regions <- variableRegions
...
variableRegions2 <- dplyr::mutate(variableRegions2,type="variable",direction=TRUE,gene=NA,molecule=NA)
tprDlocus2 <- tprDlocusGFF
tprDlocus2$type <- NULL
tprDlocus2 <- dplyr::mutate(tprDlocus2,region=NA,count=NA,length=abs(end-start),type="gene",percentage=NA)
...
ggplot(total,aes(xmin = start, xmax = end,y=molecule,label=gene)) + 
  ...
  scale_fill_manual(values = c("TP0131" = "#e41a1c", "TP0143" = "#999999", "TP0140" = "#999999",
                               "TP0136" = "#999999", "TP0133" = "#999999", "TP0134" = "#999999",
                               "TP0138" = "#999999", "TP0139" = "#999999", "TP0126" = "#999999", 
                               "TP0142" = "#999999", "TP0141" = "#999999", "TP0126c" = "#999999",
                               "TP0130" = "#999999", "TP0129" = "#999999", "TP0127" = "#999999", 
                               "TP0128" = "#999999", "TP0126d" = "#999999", 
                               "V1" = "#e41a1c", "V2" = "#377eb8", "V3" = "#4daf4a", "V4" = "gold3",
                               "V5" = "#ff7f00","V6" = "#984ea3","V7" = "#a65628"), 
                    breaks = region, labels=region, name = "Region") + theme(legend.position = c(0.9,0.6)) + 
  geom_gene_arrow(data=subset(total,type=="gene"),
                  aes(xmin = start, xmax = end,y=molecule, fill=gene, alpha=0.7, forward = direction)) + 
  geom_gene_label(align = "centre", min.size=2) + scale_alpha(guide = 'none') + ylab("Usage (sum percentage)")
...
####
groupblast <- blasthits_100 %>% group_by(qseqid) %>% dplyr::summarise(Max_End=max(qend),Min_Start=min(qstart))
...
allvariable <- allvariable %>% dplyr::rename(qseqid = seq.name)
...
allvariable_og <- allvariable_og %>% dplyr::rename(qseqid = seq.name)
...
variableRegions_order3 <- variableRegions_order2 %>% dplyr::group_by(sequence) %>% dplyr::mutate(rankpercentpercent = rankpercentage/sum(rankpercentage)*100)
